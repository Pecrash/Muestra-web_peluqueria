<footer class="secFooter">
    <article class="secFooter__content">
        <h1 class="secFooter__content-tittle">Networks</h1>
        <div class="secFooter__content-media">
            <a href=""><img src="../assets/social_icons/facebook.svg" alt=""></a>
            <a href=""><img src="../assets/social_icons/instagram.svg" alt=""></a>
            <a href=""><img src="../assets/social_icons/twitter.svg" alt=""></a>
            <a href=""><img src="../assets/social_icons/linkedin.svg" alt=""></a>
        </div>
    </article>
    <article class="secFooter__content">
        <h1 class="secFooter__content-tittle">Our Contacts</h1>
        <section class="secFooter__content-info">
            <p>Villa Salinas</p>
            <p>Carrer Negrón, 20, Bajos</p>
            <p>0491 570 006</p>
        </section>
    </article>
    <article class="secFooter__content">
        <h1 class="secFooter__content-tittle">All Services</h1>
        <section class="secFooter__content-info">
            <p>Hair Cut</p>
            <p>Shaving</p>
            <p>Drying Hair</p>
        </section>
    </article>
    <article class="secFooter__content">
        <h1 class="secFooter__content-tittle">Navigation</h1>
        <section class="secFooter__content-info">
            <p>About us</p>
            <p>our gallery</p>
            <p>contact us</p>
        </section>
    </article>
</footer>
</body>